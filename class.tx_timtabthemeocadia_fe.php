...
***************************************************************/
/**
 * class.tx_timtabthemeocadia_fe.php
 *
...
class tx_timtabthemeocadia_fe extends tslib_pibase {
	var $cObj; // The backReference to the mother cObj object set at call time
	// Default plugin variables:
	var $prefixId 		= 'tx_timtabthemeocadia_fe';		// Same as class name
	var $scriptRelPath 	= 'class.tx_timtabthemeocadia_fe.php';	// Path to this script relative to the extension dir.
	var $extKey 		= 'timtab_theme_ocadia';	// The extension key.
	...
	function substituteMarkers() {
		if($this->calledBy == 'tt_news') {	
			$this->markerArray['###OCADIA_POST_ID###'] = $this->conf['data']['uid'];
			$this->markerArray['###OCADIA_IN###'] = $this->pi_getLL('posted_in');
			$this->markerArray['###OCADIA_BY###'] = sprintf($this->pi_getLL('by'), $this->conf['data']['author']);
			$this->markerArray['###OCADIA_COMMENT_LINK###'] = $this->getCommentLink($this->conf['data']);		
		} elseif($this->calledBy == 've_guestbook') {		
			$this->markerArray['###OCADIA_SAID###'] = $this->pi_getLL('said');
			$this->markerArray['###OCADIA_COMMENTS###'] = $this->pi_getLL('comments');
		}
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_ocadia/class.tx_timtabthemeocadia_fe.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_ocadia/class.tx_timtabthemeocadia_fe.php']);
}
